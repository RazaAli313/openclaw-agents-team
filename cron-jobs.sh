...
if job_exists "task-watcher"; then
  ...
  $OC cron add \
    ...
    --no-deliver \
    --message "TASK_WATCHER_MODE: Query Notion for ALL tasks with Status='To-Do'. For each To-Do task: check task-registry.json to skip already-dispatched tasks. If Priority=High: (1) Update Notion status to 'In progress', (2) Create a Notion result page titled with the task name, (3) Write the appropriate urgent flag file (tasks/urgent-research.flag for Research agent, tasks/urgent-content.flag for Content agent) with TASK_TITLE, NOTION_PAGE_ID, NOTION_TASK_ID, ITERATION_NUM=1, USER_INSTRUCTION from task Notes or title, FEEDBACK empty, (4) Update task-registry.json with the new entry, (5) Send Telegram notification. If Priority=Medium or Low: write to tasks/research-task.md or tasks/content-task.md only (no flag file), keep Notion status as 'To-Do'. If no To-Do tasks found, stop immediately." \
...
if job_exists "telegram-listener"; then
  ...
  $OC cron add \
    ...
    --session isolated --agent commander \
    --no-deliver \
    --message "TELEGRAM_LISTENER_MODE: Read memory/telegram-offset.txt to get the last processed update ID (default 0 if file missing). Call Telegram getUpdates API: curl -s 'https://api.telegram.org/bot\$TELEGRAM_BOT_TOKEN/getUpdates?offset=OFFSET&timeout=0'. Parse the response. For each new message from chat ID matching \$TELEGRAM_CHAT_ID: (1) If it looks like a new task (e.g. 'Find me X', 'Create content for Y', 'Research Z') — create a Notion result page, write the appropriate urgent flag file (tasks/urgent-research.flag or tasks/urgent-content.flag), update task-registry.json, send Telegram confirmation. (2) If it looks like feedback/iteration (e.g. 'too expensive', 'add more', 'redo this') — read task-registry.json to find most recent matching task, write iteration flag file with existing NOTION_PAGE_ID and incremented ITERATION_NUM and the FEEDBACK text, send confirmation. (3) If it is /status — read task-registry.json and reply with current task statuses. Save the highest update_id + 1 to memory/telegram-offset.txt. If no new messages, stop immediately." \
...
if job_exists "urgent-research"; then
  ...
  $OC cron add \
    ...
    --no-deliver \
    --message "URGENT_MODE: First check if tasks/urgent-research.flag exists. If it does NOT exist, stop immediately — do nothing else. If it DOES exist: source the flag file to get TASK_TITLE, NOTION_PAGE_ID, NOTION_TASK_ID, ITERATION_NUM, USER_INSTRUCTION, FEEDBACK. Execute research based on USER_INSTRUCTION using YouTube API and Brave Search. If FEEDBACK is not empty, focus the research on addressing the feedback. Collect real data. Then: if NOTION_PAGE_ID is empty, create a new Notion page titled TASK_TITLE. If NOTION_PAGE_ID is set, append a new section (Section ITERATION_NUM) to that existing page. Add a structured table with your research results to the Notion page. Delete tasks/urgent-research.flag. Update task-registry.json status to complete. If NOTION_TASK_ID is a valid UUID, update Notion task status to 'Done'." \
...
if job_exists "urgent-content"; then
  ...
  $OC cron add \
    ...
    --no-deliver \
    --message "URGENT_MODE: First check if tasks/urgent-content.flag exists. If it does NOT exist, stop immediately — do nothing else. If it DOES exist: source the flag file to get TASK_TITLE, NOTION_PAGE_ID, NOTION_TASK_ID, ITERATION_NUM, USER_INSTRUCTION, FEEDBACK. Execute the content task based on USER_INSTRUCTION. If image generation is needed: generate with DALL-E, download the image, upload to Google Drive 'OpenClaw Content' folder (use cached folder ID from memory/drive-folder-id.txt or create folder if needed), get shareable link. Then: if NOTION_PAGE_ID is empty, create a new Notion page titled TASK_TITLE. If NOTION_PAGE_ID is set, append new section to that page. Add a structured content table (with Google Drive links for images) to the Notion page. Delete tasks/urgent-content.flag. Update task-registry.json status to complete. If NOTION_TASK_ID is a valid UUID, update Notion task status to 'Done'." \
